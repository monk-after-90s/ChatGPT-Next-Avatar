...
import {base_url} from "@/app/api/digital-man/base_url";
...
function feed2digman(text: string) {
    ...
    if (!digmanStreamId) {
        ...
        const selectElement = document.getElementById("digman-select") as HTMLSelectElement;
        const selectedOption: HTMLOptionElement | null = selectElement.options[selectElement.selectedIndex];
        if (selectedOption) {
            ...
            fetch(`${base_url}/avSustainStream/establish_stream?` + params.toString())
                ...
                .then(async streamId => {
                    ...
                    getVd().src = `${base_url}/avSustainStream/listen_video_stream?stream_id=${encoded_stream_id}`;
                    //消费文本，进行数字人合成
...
async function startConsumeText(encoded_stream_id: string) {
    ...
    while (true) {
        ...
        if (text) {
            ...
            ad.src = `${base_url}/avSustainStream/talk?stream_id=${encoded_stream_id}&speech_content=${text}`;
            ad.play();
            //确保顺序后，可以及时切换的时机
